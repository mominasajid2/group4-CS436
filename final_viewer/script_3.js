...
function navigateToConnectedCamera(clickX, clickY) {
  ...
  const xNorm = (clickX - width / 2) / (width / 2); // -1 → left, +1 → right
  ...
  const candidates = neighbors.map((idx) => {
    ...
    const dx = offset.x;
    const dz = offset.z;
    const dirScore = Math.sign(xNorm) * dx; // favour left/right roughly

    const dist = offset.length();
    const totalScore = dist - 0.5 * dirScore; // small directional bias
  ...
  // choose the best candidate not yet visited, else any
  const unvisited = candidates.map(c => c.idx).filter(n => !visited.has(n));
  let targetIdx = unvisited.length > 0 ? unvisited[0] : candidates[0].idx;
